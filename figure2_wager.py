...
def generate_data(x_points, y_true, noise_variance=0.25, seed=None):
    ...
    np.random.seed(seed)
    noise = np.random.normal(0, np.sqrt(noise_variance), len(x_points))
    return y_true + noise
...
def bagging_decision_trees(x_points, y_noisy, n_bootstrap, max_leaf_nodes=5, seed=None):
    ...
    tree_predictions_b = np.zeros((n_bootstrap, n_data_points))
...
def inf_JK_bagged_variance(N_bi, tree_predictions_b):
    ...
    N_star_mean = np.mean(N_bi, axis=0)
    T_N_star_mean = np.mean(tree_predictions_b, axis=0)

    # Calculate covariance and bias correction
    cov_matrix = np.mean((N_bi[:, :, None] - N_star_mean) * (tree_predictions_b[:, None, :] - T_N_star_mean), axis=0)
    bias_correction = (n_data_points / n_bootstrap) * np.mean((tree_predictions_b - T_N_star_mean) ** 2, axis=0)
...
def main():
    ...
    # abspeichern mit namen figure2_wager+ n_datapoints+ n_simulations + n_bootstrap + seed 
