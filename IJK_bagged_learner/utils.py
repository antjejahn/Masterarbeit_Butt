...
def inf_JK_bagged_variance(
    ...
    """
    Calculate the infinitesimal jackknife (inf-JK) bagged variance.

    Args:
        N_bi (np.ndarray): Array of shape (B, n), count-vector.
        T_N_b (np.ndarray): Array of shape (B, n_preds), preds of Decision Trees.
        weights (np.ndarray, optional): Array of shape (n,) representing the weights for each bootstrap sample.

    Returns:
        np.ndarray: Array of shape (n_preds,) representing the inf-JK bagged variance for each prediction.
    ...
    if weights is None:
        ...
        cov_vector = (
            np.sum(((N_bi - 1).T @ (T_N_b - T_N_star_mean)) ** 2, axis=0) / (B - 1) ** 2
        )

        bias_correction = (((n - 1) * B) / (B - 1) ** 2) * np.var(T_N_b, axis=0, ddof=1)
...
def inf_JK_bagged_variance_simple(
    N_bi: np.ndarray, T_N_b: np.ndarray, weights: np.ndarray = None
) -> np.ndarray:
    ...
    if weights is None:
        ...
        for i in range(n):
            for pred in range(n_preds):
                cov_i[i, pred] = (
                    (1 / weights[i])
                    * np.dot((N_bi[:, i] - 1), (T_N_b[:, pred] - T_N_star_mean[pred]))
                    / (B - 1)
                )
...
def create_bootstrap_indices_and_Nbi(
    ...
    if weights is None:
        ...
        rng = np.random.default_rng(seed)
        boot_indices = rng.choice(np.arange(n), size=(B, n), p=weights)
        boot_counts = np.apply_along_axis(
            lambda x: np.bincount(x, minlength=n), axis=1, arr=boot_indices
        )
        return boot_indices, boot_counts
...
def simulate_bagging_and_variance(
    ...
    """
    Simulates bagging and calculates variance.

    Args:
        n (int): Number of data points.
        B (int): Number of bootstrap samples.
        new_data (np.ndarray): New data points.
        simulation_index (int): Index of the simulation.
        seed (int): Seed for random number generation.
        dt_args (Dict): Arguments for decision tree.
        ...
        ijk_calculation (bool, optional): Whether to perform ijk calculation. Defaults to False.
        weights (np.ndarray, optional): Weights for bagging. Defaults to None.

    Returns:
        Tuple[np.ndarray, np.ndarray]: Bagged predictions and estimated variances.
    """
