...
def simulate_and_predict(i, n_data_points, x_points, y_true, n_bootstrap,seed):
    np.random.seed(seed)
    ...
    for b in range(n_bootstrap):
        ...
        tree_model.fit(x_points[indices].reshape(-1, 1), y_noisy[indices])
        tree_predictions[b, :] = tree_model.predict(x_points.reshape(-1, 1))
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        futures = [executor.submit(simulate_and_predict, i, n_data_points, x_points, y_true, n_bootstrap, seeds[i]) for i in range(n_simulations)]
