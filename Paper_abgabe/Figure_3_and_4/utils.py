...
def create_bootstrap_indices_and_Nbi(
    n: int, B: int, seed: int = None,weighted_boot_samples: bool=True, boot_weights: np.ndarray = None
):
    if weighted_boot_samples == False:
        rng = np.random.default_rng(seed)
...
def bagging_decision_trees(
    ...
    dt_args: Dict,
    seed: int = None,
    weighted_boot_samples: bool = True,
    ...
    indices_list, N_bi = create_bootstrap_indices_and_Nbi(
        n=n, B=B, seed=seed,weighted_boot_samples=weighted_boot_samples, boot_weights=boot_weights
    )
...
def simulate_bagging_and_variance(
    ...
    if weighted_boot_samples:
        ...
        rng = np.random.default_rng(seed)
        weights = np.zeros(n)
        keep_idx = rng.choice(n, size=n//2, replace=False)
        weights[keep_idx] = rng.random(size=len(keep_idx))
        weights /= weights.sum()
    else:
        weights = np.ones(n) / n
    ...
    tree_predictions_b, N_bi = bagging_decision_trees(
        ...
        dt_args=dt_args,
        seed=adjusted_seed,
        weighted_boot_samples=weighted_boot_samples,
    ...
    if ij_w_variance_calc:
        biased_var_estimate_w, bias_correction_w = ij_w_variance(N_bi=N_bi, T_N_b=tree_predictions_b, boot_weights=weights)
    else:
        biased_var_estimate_w = np.zeros(n)
        bias_correction_w = np.zeros(n)

    return bl_predictions, biased_var_estimate, bias_correction, biased_var_estimate_w, bias_correction_w 
