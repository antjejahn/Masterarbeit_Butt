...
def generate_data(x_points, y_true, noise_variance=NOISE_VARIANCE, seed=None):
    ...
    np.random.seed(seed)
    noise = np.random.normal(0, np.sqrt(noise_variance), len(x_points)).astype(
        np.float32
    )
    return y_true + noise
...
def create_bootstrap_indices_and_Nbi(n_data_points, n_bootstrap, seed=None):
    ...
    indices_list = np.random.choice(
        n_data_points, (n_bootstrap, n_data_points), replace=True
    )
...
def bagging_decision_trees(
    x_points, y_noisy, n_bootstrap, max_leaf_nodes=MAX_LEAF_NODES, seed=None
):
    ...
    indices_list, N_bi = create_bootstrap_indices_and_Nbi(
        n_data_points=n_data_points, n_bootstrap=n_bootstrap, seed=seed
    )

    for b in range(n_bootstrap):
        tree_model = DecisionTreeRegressor(max_leaf_nodes=max_leaf_nodes)
        tree_model.fit(
            x_points[indices_list[b]].reshape(-1, 1), y_noisy[indices_list[b]]
        )
...
def simulate_bagging_and_variance(
    x_points, y_true, n_bootstrap, simulation_index, seed
):
    """Simulate bagging and calculate variance for a single run."""
    y_noisy = generate_data(
        x_points, y_true, noise_variance=NOISE_VARIANCE, seed=seed + simulation_index
    )

    # Perform bagging
    tree_predictions_b, N_bi = bagging_decision_trees(
        x_points, y_noisy, n_bootstrap, seed=seed + simulation_index
    )
...
def save_results_png(
    ...
    plt.legend()
    plt.savefig(
        f"figure2_wager/figure2_wager_seed_{seed}_nx{n_data_points}_nB{n_bootstrap}_{int(time.time())}.png"
    )
